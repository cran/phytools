...
map.overlap<-function(tree1,tree2,tol=1e-6){
	if(!all.equal.phylo(tree1,tree2,tolerance=tol)) stop("mapped trees must have the same underlying structure")
	if(is.null(tree1$maps)||is.null(tree2$maps)) stop("tree should be a simmap modified 'phylo' object")
	overlap<-0
	for(i in 1:nrow(tree1$edge)){
		XX<-matrix(0,length(tree1$maps[[i]]),2,dimnames=list(names(tree1$maps[[i]]),c("start","end")))
		...
		YY<-matrix(0,length(tree2$maps[[i]]),2,dimnames=list(names(tree2$maps[[i]]),c("start","end")))
		...
		for(j in 1:nrow(XX)){
			...
			for(k in lower:upper){
				if(rownames(XX)[j]==rownames(YY)[k])
					overlap<-overlap+min(YY[k,2],XX[j,2])-max(YY[k,1],XX[j,1])
			}
		}
	}
