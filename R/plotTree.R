...
plotTree<-function(tree,color=NULL,fsize=1.0,ftype="reg",lwd=2,pts=TRUE){
	if(class(tree)=="multiPhylo"){
		par(ask=TRUE)
		if(!is.null(color)) names(color)<-"1"
		for(i in 1:length(tree)) plotTree(tree[[i]],color=color,fsize=fsize,ftype=ftype,lwd=lwd,pts=pts)
	} else {
