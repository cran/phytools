...
evolvcv.lite<-function(tree,X,maxit=2000,tol=1e-10){
	...
	# now optimize models
	res1<-optim(runif(3)*c(sv1,sv2,sr),lik1,C=C,D=D,y=y,
		method="L-BFGS-B",lower=tol+c(0,0,-1),upper=c(Inf,Inf,1)-tol)
	if(inherits(tree,"simmap")){
		res2<-optim(runif(2*p+1)*c(rep(c(sv1,sv2),p),sr),lik2,C=mC,
			D=D,y=y,method="L-BFGS-B",lower=tol+c(rep(0,2*p),-1),
			upper=c(rep(Inf,2*p),1)-tol)
		res3<-optim(runif(2+p)*c(sv1,sv2,rep(sr,p)),lik3,C=mC,D=D,
			y=y,method="L-BFGS-B",lower=tol+c(0,0,rep(-1,p)),
			upper=c(Inf,Inf,rep(1,p))-tol)
		res4<-optim(runif(3*p)*c(rep(c(sv1,sv2),p),rep(sr,p)),lik4,C=mC,
			D=D,y=y,method="L-BFGS-B",lower=tol+c(rep(0,2*p),rep(-1,p)),
	...
	obj<-if(inherits(tree,"simmap")) 
		list(model1=m1,model2=m2,model3=m3,model4=m4) else list(model1=m1)
