...
read.simmap<-function(file="",text,format="phylip",rev.order=TRUE,version=1.0){
	if(version<=1.0) phy<-scmap1(file,text,format,rev.order)
	if(version>1.0) phy<-scmap2(file)
	return(phy)
}

# the following is used for SIMMAP v1.0 style trees
# written by Liam J. Revell 2011

scmap1<-function(file="",text,format="phylip",rev.order=TRUE,version=1.0){
...
# the following is used for SIMMAP v1.5 style trees
# written by Liam J. Revell 2012

scmap2<-function(file=""){	
	# read modified nexus block
	# this code is adapted from read.nexus() in the ape package (Paradis et al. 2004)
	X<-scan(file=file,what="",sep="\n",quiet=TRUE)
	left<-grep("\\[",X)
	...
	for(h in 1:Ntree){
			...
			if(tree[i]=="("){
				...
				if(is.na(match(tree[i],c("(",")",",",":",";")))){
					...
					if(tree[i]==":"){
						...
						if(tree[i]=="["){
							...
							temp<-vector(); m<-1; i<-i+2
							while(is.na(match(tree[i],c(",",")")))){
								temp[m]<-tree[i]; i<-i+1; m<-m+1
							}
							maps[[j]][l]<-as.numeric(paste(temp,collapse=""))-sum(maps[[j]])
							names(maps[[j]])[l]<-as.character(state)
						}
						edge.length[j]<-sum(maps[[j]]) # create branch length
					}	
				} else if(tree[i]=="("){
				...
				if(tree[i]==":"){
					...
					if(tree[i]=="["){
						...
						maps[[match(currnode,edge[,2])]][l]<-as.numeric(paste(temp,collapse=""))-sum(maps[[match(currnode,edge[,2])]])
						names(maps[[match(currnode,edge[,2])]])[l]<-as.character(state)
					}
					edge.length[match(currnode,edge[,2])]<-sum(maps[[match(currnode,edge[,2])]]) # create branch length
				}	
				currnode<-edge[match(currnode,edge[,2]),1] # move down the tree
		...
		# untranslate
		phy[[h]]$tip.label<-trans[phy[[h]]$tip.label]
		# reverse order of times on each branch (we read them in backwards
		phy[[h]]$maps<-lapply(phy[[h]]$maps,function(x) x<-x[length(x):1])
		attr(phy[[h]],"map.order")<-"right-to-left"
	}
	# if only one tree
